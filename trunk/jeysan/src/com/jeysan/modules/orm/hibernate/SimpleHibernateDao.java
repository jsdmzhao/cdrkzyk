 ...
 * 
 * $Id: SimpleHibernateDao.java 1205 2010-09-09 15:12:17Z shally $
 */
...
public class SimpleHibernateDao<T, PK extends Serializable> {
	...
	public void save(final Collection<T> entities) {
		...
		for(T entity:entities)
			getSession().saveOrUpdate(entity);
		logger.debug("save entity: {}", entities);
	}
	...
	public void updateOnly(final T entity) {
		Assert.notNull(entity, "entity不能为空");
	...
	public Query createSqlQuery(final String queryString, final Object... values) {
		Assert.hasText(queryString, "queryString不能为空");
		Query query = getSession().createSQLQuery(queryString).setResultTransformer(CriteriaSpecification.ALIAS_TO_ENTITY_MAP);
		if (values != null) {
			for (int i = 0; i < values.length; i++) {
				query.setParameter(i, values[i]);
			}
	 ...
	 * 与find()函数可进行更加灵活的操作.
	 * 
	 * 解决包含外连接的hql语句,比如 person.sex
	 * 
	 * @param criterions 数量可变的Criterion.
	 */
	public Criteria createCriteria(final Criterion... criterions) {
		Criteria criteria = getSession().createCriteria(entityClass);
		return HibernateUtils.checkCriteria4SubQuery(criteria, criterions);
	}
