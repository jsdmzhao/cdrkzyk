...
public class DoubleCheckManager {
	...
	public List<DoubleCheck> getDoubleChecksByYear(Integer year){
		return doubleCheckDao.find("from DoubleCheck where year = ? order by seq ", year);
	}
	@Transactional(readOnly = true)
	public DoubleCheck getDoubleCheckByYear(Integer year,Integer seq){
		List<DoubleCheck> dcs = doubleCheckDao.find("from DoubleCheck where year = ? and seq = ? ", year  , seq);
