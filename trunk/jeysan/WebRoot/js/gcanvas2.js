...
(function() {
	...
	var Canvas = function(id, w, h) {
		...
		this.fgSize = function(size) {
			fgSize = (size ? size: fgSize);
			return fgSize;
		}
		...
		this.point = function(x, y) {
			...
			if (!allowOutside 
				...
				return;
			cache.push("<div style='width:"+fgSize+"px;height:"+fgSize+"px;position:absolute;left:" + x +"px;top:" + y + "px;background-color:" + fgColor + ";font-size:0px;z-index:999;'></div>");
		}
		 ...
		 *
		 */
		this.polyline = function(xy_array){
			for(var i=0;i<xy_array.length;i+=2){
				if(i+3<xy_array.length){
					line_DDA(parseInt(xy_array[i]),parseInt(xy_array[i+1]),parseInt(xy_array[i+2]),parseInt(xy_array[i+3]));
				}
			}
		}
		/**
		 ...
		 *
		 */
		this.rectangle = function(_x0,_y0,_x1,_y1){
			var x0 = parseInt(_x0),y0= parseInt(_y0),x1= parseInt(_x1),y1= parseInt(_y1);
			line_DDA(x0, y0, x1, y0);
			line_DDA(x1, y0, x1, y1);
			line_DDA(x1, y1, x0, y1);
			line_DDA(x0, y1, x0, y0);
		}
...
function drawPolyline(gc,id) {
	var coords = $('#'+id).attr('coords');
	var xy_array = coords.split(',');
	gc.polyline(xy_array);
	gc.paint();
}
function drawRectangle(gc,id){
	var coords = $('#'+id).attr('coords');
	var xy_array = coords.split(',');
	if(xy_array.length >= 4){
		gc.rectangle(xy_array[0],xy_array[1],xy_array[2],xy_array[3]);
		gc.paint();
	}else{
		alert('画矩形要两个点');
	}
