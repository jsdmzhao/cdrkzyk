...
<select style="width:<%=width==null?100:width%>px" class="<c:if test="${!noRender}">combox</c:if> <c:if test="${!empty(class1)}">${class1}</c:if>" name="${name}" id="selector_${name}" value="${value}" <c:if test="${!empty(onChange)}">change="${onChange}"</c:if> <c:if test="${readOnly}">readOnly="readOnly"</c:if>>
