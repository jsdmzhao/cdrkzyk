...
public class PolicyBabyCheck extends BaseCheck{
	
	private Logger logger = Logger.getLogger(PolicyBabyCheck.class);
	
	private static final String TABLE_IN_DB = "TEMP_POLICE_DATA_BABY";
	/**字段*/
	private static final String COLUMN_IN_DB = "NAME,SEX,CODE,BIRTHDAY,ADDRESS,CUSTODIAN_MOTHER,CUSTODIAN_FATHER,UPDATE_TIME,ID";
	
	private static final String INSERT_SQL_4NEWDATA_1 = "insert into FHP_PERSON(NAMEH,SEX,CODE) VALUES (?,?,?)";
	private static final String INSERT_SQL_4NEWDATA_2 = "insert into FHP_PERSON_BASIC(PERSON_ID,ADDRESS,BIRTHDAY,FATHER,MOTHER) select id,?,?,?,? from FHP_PERSON where code = ?";
	private static final String INSERT_SQL_4NEWDATA_3 = "insert into FHP_WOMAN_CHILDREN(PERSON_SELF_ID,NAMEH,SEX,BIRTHDAY,REMARK) select id,?,?,?,? from FHP_PERSON where code = ?";
	
	private static final String UPDATE_SQL_4UPDATEDATA_1 = "update FHP_PERSON set NAMEH = ? , SEX = ? where CODE = ? ";
	private static final String UPDATE_SQL_4UPDATEDATA_2 = "update FHP_PERSON_BASIC set ADDRESS = ?,BIRTHDAY = ?,FATHER = ?,MOTHER = ? where PERSON_ID = (select ID from FHP_PERSON where CODE = ? limit 1)";
	private static final String UPDATE_SQL_4UPDATEDATA_3 = "update FHP_WOMAN_CHILDREN set NAMEH = ?,SEX = ?,BIRTHDAY = ?,REMARK = ? where PERSON_SELF_ID = (select ID from FHP_PERSON where CODE = ? limit 1)";
	
	private static List tmpIds4Update = new ArrayList();
	
	public PolicyBabyCheck(Connection conn){
		super(conn);
	}
	...
	private void addNewData(JdbcUtil jdbcUtil,boolean update) throws Exception{
		logger.info(String.format("开始%s公安部门 婴儿出生入户数据......",update?"修改":"新增"));
		try{
			List<Object[]> data4new = getData4Todo(update);
			if(data4new!=null && data4new.size() > 0){
				String name , code ,address , father,mother;
				Integer sex;
				Date birthday;
				Long id;
				...
				for(Object[] m : data4new){
					...
					params_1.add(new Object[]{name,sex,code});
					params_2.add(new Object[]{address,birthday,father,mother,code});
					params_3.add(new Object[]{name,sex,birthday,String.format("母亲姓名:%s,父亲姓名:%s",mother,father),code});
			...
			logger.info(String.format("%s公安部门 婴儿出生入户数据成功......",update?"修改":"新增"));
		}catch(Exception e){
			logger.error(String.format("%s公安部门 婴儿出生入户数据出错......",update?"修改":"新增"),e);
			throw e;
		}
	}
