...
<%@ attribute name="hasButton" type="java.lang.Boolean" required="false" description="是否需要btnLook默认需要"%>
<c:if test="${empty id}">
...
onclick="javascript:pdialogOpen('${ctx}/district/districtprovince!districtselector.action?label4id=${id}_code_&label4name=${id}_name_&value=${value}&onChange=${onChange}&type=${type}', '定位地址');"/>
<c:if test="${empty hasButton || hasButton}">
<a class="btnLook"
