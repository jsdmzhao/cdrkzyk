...
<select style="width:<%=width==null?100:width%>px" class="combox <c:if test="${!empty(class1)}">${class1}</c:if>" name="${name}" value="${value}" <c:if test="${!empty(onChange)}">change="${onChange}"</c:if> <c:if test="${readOnly}">readOnly="readOnly"</c:if>>
