...
"")+(l?' width="'+l+'"':"")+">"});b=b.replace(/\[\/(table|tr|td)\]/ig,"</$1>");b=b.replace(/\[\*\]((?:(?!\[\*\]|\[\/list\]|\[list\s*(?:=[^\]]+)?\])[\s\S])+)/ig,"<li>$1</li>");b=b.replace(/\[list\s*(?:=\s*([^\]"]+?)(?:"[^\]]*?)?)?\s*\]/ig,function(a,h){var i="<ul";if(h)i+=' type="'+h+'"';return i+">"});b=b.replace(/\[\/list\]/ig,"</ul>");for(g=1;g<=n;g++)b=b.replace("[\tubbcodeplace_"+g+"\t]",m[g]);return b=b.replace(/(^|<\/?\w+(?:\s+[^>]*?)?>)([^<$]+)/ig,function(a,h,i){return h+i.replace(/[\t ]/g,
function(l){return{"\t":"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"," ":"&nbsp;"}[l]})})}
